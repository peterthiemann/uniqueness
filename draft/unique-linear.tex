...
\begin{mathpar}
  \inferrule{
    ...
    A \vdash \REC{ \overline{a_i=e_i}} : \REC{ \overline{a_i : t_i}}
  }
\end{mathpar}
...
\begin{mathpar}
  \inferrule{
    ...
    A \vdash \UPD {e_0}a{e_1} : \REC{ a:t \mid r}
  }
\end{mathpar}
...
\begin{itemize}
\item $O\subseteq \{\READ\}$: $Q = O \cup P$,
  no further action required as potential writing in $P$ happens after reading
\item $\WRITE \in O$, $P \subseteq \{\READ\}$: two choices, not clear
  which is better
  \begin{itemize}
  ...
  \end{itemize}
\item $\WRITE\in O$, $\WRITE \in P$: $Q = \WRITE$ insert copy,
