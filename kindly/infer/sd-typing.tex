\begin{mathpar}
  \inferrule[Var]
  { \bvar{x}{
      \forall \kvar_i \forall (\tvar_j:k_j).\ \qual{C_x}{\tau}}
    \in \E \\
    \unif = [\kvar_i\mapsto k_i,\tvar_j \mapsto \tau_j] \\
    \entail C {\unif(C_x)} \\
    \addlin{
      \inferSK{C}{\E}{\unif \tau}{k_\tau}
    }
  }
  { \inferS{C}{\E}{x}{\unif\tau}
  }
  \and
  \inferrule[Abs]
  { \tvar,\kvar_\tvar,\kvar\text{ new}\\
    \inferW{\Sv'}{(C',\unif')}
    {\E;\bvark{x}{\tvar}{\kvar_\tvar}}{e}{\tau} \\\\
    \addlin{ \Sv = \Sv' \Sdel{x} }\\
    D = C'\Cand
    \addlin{\Cleq{\Sv}{\kvar} \Cand \Weaken_{(x,\kvar_\tvar)}(\Sv')} \\
    (C,\unif) = \normalize{D}{\unif'}
  }
  { \inferW{\addlin{\Sv}}{(C,\unif\Sdel{\tvar})}{\E}
    {\lam{x}{e}}{\unif(\tvar)\tarr{\kvar}\tau} }
  \and
  \inferrule[App]
  { 
    \inferS{C}{\E}{e_1}{\tau_2 \tarr{k} \tau_1} \\
    \inferS{C}{\E}{e_2}{\tau'_2} \\
    \addlin{\tsplit{C}{\Sv}{\Sv_1}{\Sv_2}}\\
    \entail C {\Cleq{\tau_2'}{\tau_2}} 
  }
  { \inferS{C}
    {\E}{\app{e_1}{e_2}}{\tau_1} }
  \and
  \inferrule[Let]
  { \inferW{\Sv_1}{(C_1,\unif_1)}{\E}{e_1}{\tau_1} \\
    (C'_1,\sigma) = \generalize{C_1}{\unif_1\E}{\tau_1} \\
    \inferW{\Sv_2}{(C_2,\unif_2)}{\E;\bvar{x}{\sigma}}{e_2}{\tau_2} \\
    \unif' = \unif_1 \mostgeneral \unif_2 \\
    \addlin{\tsplit{C_s}{\Sv}{\Sv_1}{\Sv_2}}\\
    D =
    C'_1 \Cand C_2 \Cand \addlin{C_s} \\
    (C,\unif) = \normalize{D}{\unif'}\\
  }
  { \inferW{\addlin{\Sv}}{(C,\unif|_{\fv{\E}})}
    {\E}{\letin{x}{e_1}{e_2}}{\unif\tau_2} }
  \and
  \inferrule[Pair]
  { \inferS{C}{\E}{e_1}{\tau_1} \\
    \inferS{C}{\E}{e_2}{\tau_2} \\
    \addlin{\tsplit{C}{\Sv}{\Sv_1}{\Sv_2}}
  }
  { \inferS{C}{\E}{\introPair{e_1}{e_2}}{\tyPair{\tau_1}{\tau_2}} }
  \and
  \inferrule[MatchPair]
  { 
    \inferS{C}{\E}{e_1}{\tyPair{\tau_1}{\tau'_1}} \\
    \inferS{C}
    {\E;\bvark{x}{\tau_1}{k};\bvark{x'}{\tau'_1}{k'}}{e_2}{\tau_2} \\
    \addlin{\tsplit{C}{\Sv}{\Sv_1}{\Sv_2}}
  }
  { \inferS{C}
    {\E}{\matchin{x,x'}{e_1}{e_2}}{\tau_2} }
  \and
  %
  \inferrule[Borrow]
  { \kvar \text{ new}\\
    \inferW{\Sv'}{(C,\unif)}{\E}{x}{\tau} \\
    % k = \operatorname{kind}(\iota)\\
    \Sv = \Sv \Sdel{x} \Sunion \Sone{\borrow{x}}{\kvar} \\
  }
  { \inferW{\Sv}{(C,\unif)}{\E}{\borrow{x}}{\borrowty{\kvar}{\tau}} }
  \and
  \inferrule[Region]
  { \inferW{\addlin{\Sv'}}{(C',\unif')}{\E}{e}{\tau} \\
    \inferK{(C_\tau,\unif_\tau)}{\E}{\tau}{k_\tau}\\
    \addlin{ \tregion{C_r}{x}{\Sv}{\Sv'} }\\
    D = C' \Cand C_\tau \Cand \Cleq{k_\tau}{\klin_{n-1}} \Cand C_r\\
    (C,\unif) = \normalize{D}{\unif' \mostgeneral \unif_\tau}\\    
  }  { \inferW{\addlin{\Sv}}{(C,\unif)}{\E}{\region{x}{e}}{\tau} }
  \and
  %
  % \inferrule[Elim]
  % { \tvar,(\kvar'_i),(\tvar'_j)\text{ new}\\
  %   \bvar{K}{
  %     \forall \kvar_i \forall (\tvar_j:\kvar_j).\ \qual{C_K}{\tau_1 \tarr{}\tau_2}
  %   } \in \E\\
  %   \inferW{\Sv}{(C,\unif)}{\E}{e}{\tau} \\
  %   \unif' =
  %   \subst{\kvar_i}{\kvar'_i}{} \meet
  %   \subst{\tvar_j}{\tvar'_j}{} \meet \unif \\
  %   D =
  %   C \Cand C_K \Cand \Cleq{\tau_1}{\tvar} \Cand \Cleq{\tau}{\tau_2} \\
  %   (C,\unif) = \normalize{D}{\unif'}\\
  % }
  % { \inferW{\addlin{\Sv}}{(C,\unif|_{\fv{\E}})}{\E}{\elimK{K}{e}}
  %   {\unif\tvar} }
\end{mathpar}

% \begin{align*}
%   \Weaken(x,\Sv)
%   &\equiv \begin{cases}
%     \operatorname{kind}(x)\lk\kun &\text{if } \operatorname{kind}(x)\in\Sv\\
%     \Cempty &\text{otherwise}
%   \end{cases}\\
%   \Cleq{\Sv}{k}
%   &\equiv \bigwedge_{\kvar\in\Sv} \Cleq{\kvar}{k}
% \end{align*}

%%% Local Variables:
%%% mode: latex
%%% TeX-master: "../main"
%%% End:
