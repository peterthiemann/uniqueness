...
\begin{frame}[t,fragile]{Borrows}
  ...
  \begin{itemize}
  \item
    \textbf{Shared} borrows \tikzcoord{UX1}@&a@\tikzcoord{UX2} are for observing the resource (Unrestricted -- @un@)
    % \textbf{Shared} borrows \tikzcoord{UX1}@&a@\tikzcoord{UX2} which are Unrestricted (@un@)
  \item
    \textbf{Exclusive} borrows \tikzcoord{AX1}@&!a@\tikzcoord{AX2} are for modifying the resource (Affine -- @aff@)
...
\end{minted}
      ...
      \draw[link,->] (S1) to node[midway,above,sloped] {1. Elaborate regions} (S2) ;
