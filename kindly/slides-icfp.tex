...
\end{minted}
  ...
  \begin{tikzpicture}[remember picture,overlay]%
    ...
    \end{onlyenv}
    \begin{onlyenv}<5->
      \draw[link,<-] (B.10) to[in=180,out=40] ++(2,1) node[annot, anchor=west] {
...
\begin{frame}[fragile,shrink]
  ...
  \begin{center}
    \begin{tikzpicture}
      \node[annot,text width=11.5cm,scale=0.5] at (180:5) (S1) {%
\begin{minted}[texcomments=true]{OCaml}
let average db subjects = 
  List.map (Dbm.find db) subjects / List.length subjects
...
\end{minted}
      ...
      \node[annot,text width=12cm,scale=0.5] at (108:3) (S2) {%
\begin{minted}[texcomments=true,escapeinside=\#\#]{OCaml}
let average db subjects =
  List.map (Dbm.find db) subjects / List.length subjects
...
\end{minted}
      ...
      \node[annot] at (36:4) (S3) {
        CONTRAINTES
      };

      \node[annot] at (-36:4) (S4) {
        SCHEMES
      };

      \node[annot] at (-108:3) (S5) {
        SIMPLIFIED SCHEMES
      };

      \draw[link,->] (S1) to[bend left] node[midway] {Infer regions} (S2) ;
      \draw[link,->] (S2) to[bend left] node[midway] {Convert into constraints} (S3);
      \draw[link,->] (S3) to[bend left] node[midway] {Solve constraints} (S4);
      \draw[link,->] (S4) to[bend left] node[midway] {Simplify types} (S5);
    \end{tikzpicture}
  \end{center}
