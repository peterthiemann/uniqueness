...
\newcommand\resultMutables[3]{
  ...
  $\Store#3 (\Loc) \ne \StFreed$
  and $\Addr\in{\Perm#3}$ implies $\Addr\in
  \Perm#1$.
}
...
\newcommand\resultResources[3]{
  % input, inputenv, output
  Resources:
  Let $\REACH#1 = \Reach{\Store#1}{\Active{\VEnv#2}}$.
  Let $\REACH#3 =\Reach{\Store#3}{\Active{\VEnv#2}}$.
  \\
  For all $\Loc$ such that $n= \MultiNumber\Loc{\REACH#1} >0$ and $n' =
  \MultiNumber\Loc{\REACH#3}$,
  it must be that  $n=n'=1$, $\Loc\notin\Perm#3$, and if
  $\Store#1(\Loc)$ is a resource, then $\Store#3 (\Loc) = \StFreed$.
  % \begin{itemize}
...
\newcommand\assumeIncoming[2]{
  ...
  \item $\forall \Loc \in \REACH
    =\Rawloc{\Reach{\Store#1}{\Suspended{\VEnv#2}}}$, 
    if $\Affine{\SE#1}\Loc$ then
    $\MultiNumber\Loc\REACH= 1$.
  \end{enumerate}
}
\begin{theorem}
  ...
  \item $\Perm$ contains at most one permission for
    each raw location
  \item\label{item:11}
    $\Rawloc\Perm \subseteq \Dom\Store \setminus \Store^{-1} (\StFreed)$
  ...
  \end{enumerate}
  For all $i\in\Nat$, if $\Store, \Perm, \VEnv \vdash {e}
  \Downarrow^i R'$ and $R'\ne \TimeOut$, 
  then
...
\begin{proof}
  ...
  \item $\Reach{\Store_1}{\Active{\VEnv_2'}, \MutableBorrows{\VEnv_2'},
    ...
    We start from~\ref{item:12}, which gives immediately
