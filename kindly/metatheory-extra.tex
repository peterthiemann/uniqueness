...
\newcommand\resultPermDom[2]{
  $\Rawloc{\Perm#2} \subseteq \Dom{\Store#2}$
}
\newcommand\resultReachPerm[2]{
  $\Reach{\Store#2}{r#2} \subseteq \Perm#2$
}
\newcommand\resultFrame[2]{
  Frame: \\
  For all $\Loc \in \Dom{\Store#1} \setminus
  \Rawloc{\Reach{\Store#2}{\VEnv#1}}$ it must be that
  \begin{itemize}
  \item $\Store#2 (\Loc) = \Store#1 (\Loc)$ and
  \item  for any $\Addr$ with $\Rawloc\Addr = \{\Loc\}$,
    $\Addr \in \Perm#1 \Leftrightarrow \Addr\in\Perm#2$.
  \end{itemize}
}
\newcommand\resultImmutables[2]{
  Unrestricted values, resources, and borrows: \\
  For all $\Addr \in
  \Reach{\Store#2}{\ImmutableBorrows{\VEnv#1}}$ with
  $\Rawloc\Addr = \{\Loc\}$, it must be that
  $\Loc\in\Dom{\Store#1}$,
  $\Store#2 (\Loc) = \Store#1 (\Loc) \ne \StFreed$
  and $\Addr\in\Perm#2$.
}
\newcommand\resultMutables[2]{
  Affine borrows and resources:\\
  For all $\Addr \in  \Reach{\Store#2}{\MutableBorrows{\VEnv#1}}$
  with $\Rawloc\Addr = \{\Loc\}$, if
  $\Loc\in\Dom{\Store#1}$ and $\Addr\ne\Loc$, then 
  $\Store#2 (\Loc) \ne \StFreed$
  and $\Addr\in{\Perm#2}$ implies $\Addr\in
  \Perm#1$.
}
\newcommand\resultSuspended[2]{
  Suspended borrows: \\
  For all $\Addr \in \Reach{\Store#2}{\Suspended{\VEnv#1}}$ with
  $\Rawloc\Addr = \{\Loc\}$ it must be that $\Addr \notin \Perm#2$.
  \begin{itemize}
...
\newcommand\resultResources[2]{
  Resources:
  Let $\REACH#1 = \Reach{\Store#1}{\Active{\VEnv#1}}$.
  Let $\REACH#2 =\Reach{\Store#2}{\Active{\VEnv#2}}$.
  \\
  For all $\Loc$ such that $n= \MultiNumber\Loc{\REACH#1} >0$ and $n' =
  \MultiNumber\Loc{\REACH#2}$,
  it must be that  $n=n'=1$, $\Loc\notin\Perm#2$, and if
  $\Store#1(\Loc)$ is a resource, then $\Store#2 (\Loc) = \StFreed$.
  % \begin{itemize}
...
\begin{proof}
  ...
  \item From~\eqref{eq:17} we have $\SE \vdash \VEnv_2 : \E_2$.
    ...
    %% PJT: generalization lemma needed 
