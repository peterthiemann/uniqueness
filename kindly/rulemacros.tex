...
\newcommand\ruleSPLam[1][i]{%
  \inferrule[SPLam]{
    ...
    \Store, \Perm, \VEnv \vdash \ilam
    {\Multi[i]{\kvar}}{\Multi[j]{\tvar}}Ck xe \Downarrow^{#1+1} \Store', \Perm\Sadd\Loc, \Loc
  }
}

\newcommand\ruleSApp[1][i]{%
  \inferrule[SApp]{
    \Store, \Perm, \VEnv \vdash e \Downarrow^{#1} \Store', \Perm', \Loc \\
    \Store' (\Loc) = (\VEnv'',\lam[k]{x}{e''}) \\
    \Perm'' = \IF{\entail {} {k \le \kun}}{\Perm'}{ \Perm'\Sdel\Loc}\\
    \Store', \Perm'', \VEnv \vdash e' \Downarrow^{#1} \Store'', \Perm''', r' \\
    \Store'', \Perm''', \VEnv''[x\mapsto r'] \vdash e'' \Downarrow^{#1}
    \Store''', \Perm'''', r
  }{\Store, \Perm, \VEnv \vdash \app{e}{e'} \Downarrow^{#1+1} \Store''',\Perm'''', r}
}

\newcommand\ruleSLet[1][i]{%
  \inferrule[SLet]{
    \Store, \Perm, \VEnv \vdash e \Downarrow^{#1} \Store', \Perm', r \\
    \Store', \Perm', \VEnv[x \mapsto r] \vdash e' \Downarrow^{#1} \Store'', \Perm'', r'
  }{
    \Store, \Perm, \VEnv \vdash \letin{x}{e}{e'} \Downarrow^{#1+1} \Store'',
    \Perm'', r'
  }
}
...
\newcommand\ruleSPair[1][i]{%
  \inferrule[SPair]{
    \Store, \Perm, \VEnv \vdash e \Downarrow^{#1} \Store', \Perm', r \\
    \Store', \Perm', \VEnv \vdash e' \Downarrow^{#1} \Store''1, \Perm'', r' \\
    \Loc\notin\Dom{\Store''} \\
    \Store''' = \Store''[\Loc \mapsto \introPair[k]r{ r'}]
  }{
    \Store, \Perm, \VEnv \vdash \introPair[k]{e}{e'} \Downarrow^{#1+1}
    \Store''', \Perm''\Sadd\Loc, \Loc
  }
}
...
\newcommand\ruleSMatchLocation[1][i]{%
  \inferrule[SMatchLocation]{
    ...
    \Perm'' = \IF{\entail {} {k \le
        \kun}}{\Perm'}{\Perm'\Sdel\Loc} \\
    \Store', \Perm'', \VEnv[x,y \mapsto \Addr', \Addr''] \vdash e'
    \Downarrow^{#1} \Store'', \Perm''', r''
  }{
...
\newcommand\ruleSMatchBorrow[1][i]{%
  \inferrule[SMatchBorrow]{
    \Store, \Perm, \VEnv \vdash e \Downarrow^{#1} \Store', \Perm', \Addr \\
    ...
    \Store, \Perm, \VEnv \vdash \matchin{x,y}{e}{e'} \Downarrow^{#1+1}
    \Store''', (\Perm''' \Sdel{\AddrD', \AddrD''}) \Sadd{\Addr', \Addr''},  r''
  }
}
...
\newcommand\ruleSRegion[1][i]{%
  \inferrule[SRegion]{
    ...
    \Store, \Perm, \VEnv \vdash \region{x}{e} \Downarrow^{#1+1} \Store', (\Perm' \Sdel{\sborrow\Addr})\Sadd\Addr, r
  }
}
...
\newcommand\ruleSBorrow[1][i]{%
  \inferrule[SBorrow]{
    ...
    \Store, \Perm, \VEnv \vdash \borrow{x} \Downarrow^{#1+1} \Store, \Perm, \sborrow\Addr
  }
}
...
\newcommand\ruleSCreate[1][i]{%
  \inferrule[SCreate]{
    \Store, \Perm, \VEnv \vdash e \Downarrow^{#1} \Store', \Perm', r\\
    \Loc\notin \Dom{\Store'} }{
    \Store, \Perm,\VEnv \vdash \create e \Downarrow^{#1+1} \Store'[\Loc
    \mapsto \rss{r}], \Perm'\Sadd\Loc, \Loc
  }
}
...
\newcommand\ruleSDestroy[1][i]{%
  \inferrule[SDestroy]{
    ...
    \Store'[\Loc\mapsto \blob], \Perm'\Sdel\Loc, ()
  }
}
...
\newcommand\ruleSObserve[1][i]{%
  \inferrule[SObserve]{
    ...
    \Store, \Perm, \VEnv \vdash \observe e \Downarrow^{#1+1} \Store', \Perm', r
  }
}
...
\newcommand\ruleSUpdate[1][i]{%
  \inferrule[SUpdate]{
    ...
    \Store, \Perm, \VEnv \vdash \update e {e'} \Downarrow^{#1+1} \Store''',
    \Perm'' \Sdel{\Addr},  ()
  }
}
