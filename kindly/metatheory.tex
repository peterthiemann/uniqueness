...
\begin{theorem}
  ...
  \item Outside: For all $\Loc \in \Dom{\Store'} \setminus \Reach{\Store'}{\VEnv}$ it must be that
    $\Store' (\Loc) = \Store (\Loc)$
    and $\Loc\in\Perm \Leftrightarrow \Loc\in\Perm'$ 
  \item Immutables: For all $\Loc \in \Reach{\Store'}{\ImmutableBorrows\VEnv}$ it must be that
    $\Store' (\Loc) = \Store (\Loc)$
    and $\Loc\in\Perm \Leftrightarrow \Loc\in\Perm'$ 
  \item Resources: For all $\Loc \in \Reach{\Store'}{\Active\VEnv}$,
    if $\Linear{\SE'}\Loc$ then $\Loc\notin\Perm'$.
  \item No thin air permission: For all $\Loc\in \Perm'$, either $\Loc \in \Dom{\Store'}
    \setminus \Dom{\Store}$ or $\Loc \in \Perm$.
  \end{itemize}
\end{theorem}
