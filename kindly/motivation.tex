...
A pool is parameterized by its content. Additional, the kind of the pool
depends on the content: Linear content implies
a linear pool, while unrestricted content implies an unrestricted pool.
The implementation uses a concurrent queue
as a bag of elements with atomic add and remove operations.
The \lstinline/Pool.create/ and \lstinline/Pool.consume/ allow to build or destroy
the pool by providing the necessary components for the elements
of the loop.
The main function of interest is the \lstinline/Pool.use/ function, which
allow to use an mutable borrow of the object. The function proceeds by removing
the element from the pool (or creating a fresh element),
passing it down to the \lstinline/f/ function, and then adding
it back to the pool.
We explicitly delimit the region on \cref{line:pool:region} to ensure that
the \lstinline/f/ function will not be able to capture \lstinline/&!o/.
...
use : ('a_1:'k_1),('a_2:'k_2),('k_1 < lin_r),('k_2 < lin_r) =>
  &('a_1 pool) -> (&!(aff_r,'a_1) -{'k}> 'a_2) -{'k_1}> 'a_2
\end{lstlisting}
    \caption{Interface}
    \label{intf:pool}
