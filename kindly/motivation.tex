...
far, such regions didn't appear in the type signature explicitly.
We know present an example implement of a \texttt{Pool} module
that uses such region to enforce linearity of the objects in the pool.
...
let create spawn =
  { spawn ; queue = Queue.create () }
let consume f c = Queue.iter f c.queue

let use { spawn ; queue } f =
  let o = match Queue.pop &queue with
    | Some x -> x
    | None () -> spawn ()
  in
  let r = f &!o in
