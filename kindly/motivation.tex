...
functions like @bfold@ and @iter_mut@. The obvious sequential implementation of
both functions boils down to a loop over the index range of the array:
\begin{lstlisting}
let rec bfold_helper f a z i =
  if i >= 0
  then bfold_helper f &a (f (get_sb (&a, i))) (i - 1)
  else z
let bfold f a z =
...
let rec iter_helper f a i =
  if i >= 0
  then (f (get_eb (&!a, i)); iter_helper f &!a (i - 1))
  else ()
let iter_mut f a =
  iter_mut f a (length &a - 1)
\end{lstlisting}
Observe that we are forced to use different primitives
\begin{itemize}
\item @get_sb : &('r, 'a Array.t) -> int -> &('r, 'a)@ to get a
  shared borrow from a shared borrow of an array and
\item @get_eb : &!('r, 'a Array.t) -> int -> &('r, 'a)@ to get an
  exclusive borrow from an exclusive borrow of  an array.
\end{itemize}
These primitives have the same underlying implementing (the same as
