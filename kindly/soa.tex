...
\paragraph{Substructural type-systems in functional languages}
...
As opposed to approaches presented previously, including \lang,
LH annotates linearity on the arrow, similarly to linear logic.
LH's ``linear arrows'', written $\multimap$ 
are functions that \emph{use} their argument exactly once
...
\item
  ...
  Only type \emph{checking} is formalized.
...
\paragraph{Other substructural type-systems}
...
to specify precisely the memory layout of objects
...
that a garbage collector is still available. This forgo numerous
...
if they are linear or unrestricted.
...
\paragraph{Type-system features}
%
