...
\begin{figure}[tp]
  \begin{center}
    ...
    \begin{tabular}{l|c@{~}cccccccccc}
      \textbf{Language}
      & UAL & \rot{Borrows} & \rot{State}
      & \rot{Resource Subsumption}
      & \rot{Resource Polymorphism}
      & \rot{Identity}
      & \rot{Concurrency}
      & \rot{Escape hatch}
      ...
      & UA & \N & \Y & \M & \Y & \Y & \Y & \Y & \MC Local & \N & ML \\
      ...
      & UL & \N & \M & \N & \Y & \N & \M & \M & \MC Non-pr. & \MC Manual & Haskell \\
      Mezzo~\citep{DBLP:phd/hal/Protzenko14,DBLP:journals/toplas/BalabonskiPP16}
      & UA & \M & \Y & \M & \Y & \Y & \Y & \Y & \MC Local & \YC Coq & ML \\
      \hline
      ...
      & UA & \N & \Y & \Y & \Y & \Y & \N & \Y & \N & \MC Manual & Java \\
      \hline
      \lang
...
Features are selected according to their relevance for resource
management and programmer convenience.
The column UAL
specifies the substructural features supported (unrestricted, affine,
linear). Borrows and state indicate support for the respective
feature. ``Resource Subsumption'' indicates that unrestricted elements 
can be promoted to affine and then linear. This promotion applies to
all objects, resources,
borrows, or closures. ``Resource Polymorphism'' refers to polymorphism over
substructural features, notably for closures.
...
``Escape hatch'' indicates whether users can (locally) opt out of
the resource management through language-integrated means such
as Rust's \texttt{unsafe}. Partial support, in the case of \lang
for instance, indicates this feature is available, but not formalized.
Type ``inference''  can be local, principal, or non-principal (if the
inferred type is not necessary the most general). ``Formalization''
...
Instead of kinds, Rust uses marker traits like \lstinline/Copy/,
\lstinline/Send/, and \lstinline/Sync/ to distinguish characteristics 
of types, which would just be unrestricted in \lang.
Also, Rust's lifetime system is more explicit and more expressive than \lang.
