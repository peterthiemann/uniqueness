...
The typing for a region (see the {\sc Region} rule in \ref{selectrules:region})
replaces suspended bindings by borrow bindings, typechecks the body
of the region, and finally ensures that the borrow does not leak outside.
This last check is done with integer indices that correspond to the nesting
level of the region. The kind of a borrow is indexed with the level
corresponding to its region and the constraint $\Cleq{\tau}{\klin_{n-1}}$ ensures that
