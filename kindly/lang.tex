...
Expressions $e$ need to be elaborted with some kind and constraint
...
location, we may take affine borrows and reborrowed. Once we have
...
@salloc : store -> storable -> (loc * store)@ such that
...
# eval : store -> perm -> venv -> int -> exp -> (store * perm * result) sem
\end{lstlisting}
...
definition for the appendix~\cref{sec:semant-defin}. The appendix also
...
printing. The pervasive \lstinline/let*/ operator acts as monadic bind
